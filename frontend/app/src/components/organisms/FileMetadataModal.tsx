...
export const FileMetadataModal = ({ metadata, onClose, onSave }: FileMetadataModalProps) => {
    ...
    >
      <div className="bg-white rounded-lg shadow-xl p-6 max-w-4xl max-h-[80vh] overflow-y-auto w-full mx-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold">File Metadata: {metadata.filename}</h3>
        ...
        <div className="space-y-4">
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  ...
                  onChange={(e) => setEditedMetadata(prev => prev ? { ...prev, name: e.target.value } : prev)}
                  className="w-full border rounded px-3 py-2"
                />
              ) : (
                <div className="w-full border rounded px-3 py-2 bg-gray-50">{editedMetadata.name}</div>
              )}
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Size</label>
              <div className="w-full border rounded px-3 py-2 bg-gray-100">{metadata.size} bytes</div>
            </div>
          </div>
          ...
          <div>
            ...
            {isEditing ? (
              <textarea
                ...
                onChange={(e) => setEditedMetadata(prev => prev ? { ...prev, description: e.target.value } : prev)}
          ...
          <div>
            <h4 className="font-medium mb-2">Columns ({metadata.columns.length})</h4>
              ...
              <table className="w-full border border-gray-300">
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="border border-gray-300 px-3 py-2 text-left">Input Type</th>
                ...
                <tbody>
                  {editedMetadata.columns.map((col, index) => (
                    <tr key={index}>
                      <td className="border border-gray-300 px-3 py-2 font-medium">{col.column}</td>
                      ...
                      <td className="border border-gray-300 px-3 py-2">
                        {isEditing ? (
                          <select
                            ...
                            onChange={(e) => updateColumn(index, 'input_type', e.target.value)}
