...
export const apiService = {
  ...
  async updateProject(projectId: string, name: string, description: string): Promise<ProjectData> {
    const payload = { name, description }
  ...
  async getSessions(projectId: string): Promise<SessionData[]> {
    ...
    const response = await fetch(`${API_BASE_URL}/projects/${projectId}/sessions`, {
      method: 'GET',
      headers: getAuthHeaders(),
    })
  ...
  async getSelectedFiles(projectId: string): Promise<FileData[]> {
    ...
    const response = await fetch(`${API_BASE_URL}/files/${projectId}/selected`, {
      method: 'GET',
      headers: getAuthHeaders(),
    })
    if (!response.ok) throw new Error(`Failed to fetch selected files: ${response.status}`)
  ...
  async uploadFile(projectId: string, file: File): Promise<void> {
    console.log('📤 API Request - Upload File:', { projectId, fileName: file.name, fileSize: file.size })

    // 1. Get presigned URL
    const urlResponse = await fetch(`${API_BASE_URL}/files/${projectId}/upload-url?filename=${encodeURIComponent(file.name)}`, {
      method: 'POST',
      headers: getAuthHeaders(),
    })
    ...
    const { file_id, url, fields }: PresignedUrlResponse = await urlResponse.json()
    console.log('✅ API Response - Get Upload URL:', { file_id, url: url.substring(0, 50) + '...', fields })
    ...
    try {
      ...
      console.log('✅ S3 Upload Response: Success (status may be blocked by CORS)')
    } catch (error) {
      console.log('⚠️ CORS error but upload likely succeeded:', error)
    }

    // 3. Confirm upload
    const confirmResponse = await fetch(`${API_BASE_URL}/files/${file_id}/confirm?size=${file.size}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    })
    ...
    if (!confirmResponse.ok) {
      ...
      console.error('❌ Confirm upload error:', confirmResponse.status, errorText)
  ...
  async updateFileStatus(fileId: string, status: string): Promise<FileResponse> {
    console.log('🔄 API Request - Update File Status:', { fileId, status })

    const response = await fetch(`${API_BASE_URL}/files/${fileId}/status?status=${status}`, {
      method: 'PATCH',
      headers: getAuthHeaders(),
    })
    if (!response.ok) throw new Error(`Failed to update file status: ${response.status}`)
  ...
  async sendMessage(sessionId: string, content: string, chatWithData: boolean = false, modelId: string = "OPENAI_20b_BR"): Promise<ChatResponse> {
  ...
  async refreshSession(sessionId: string): Promise<SessionData> {
    ...
    const response = await fetch(`${API_BASE_URL}/sessions/${sessionId}/refresh`, {
      method: 'GET',
      headers: getAuthHeaders(),
    })
    if (!response.ok) throw new Error(`Failed to refresh session: ${response.status}`)
  ...
  async getSessionWithMessages(sessionId: string): Promise<SessionWithMessagesResponse> {
    ...
    if (!response.ok) {
      const errorText = await response.text()
      console.error('❌ Get session with messages error:', response.status, errorText)
      throw new Error(`Failed to get session with messages: ${response.status}`)
    }
  ...
  async renameFile(fileId: string, newFilename: string): Promise<FileResponse> {
    ...
    const response = await fetch(`${API_BASE_URL}/files/${fileId}/status?status=renamed`, {
      method: 'PATCH',
      headers: getAuthHeaders(),
    })
  ...
  async updateFileMetadata(fileId: string, metadata: FileMetadata): Promise<FileResponse> {
    console.log('📊 API Request - Update File Metadata:', { fileId, metadata })

    const response = await fetch(`${API_BASE_URL}/files/${fileId}/metadata?name=${encodeURIComponent(metadata.name || '')}&description=${encodeURIComponent(metadata.description || '')}`, {
      method: 'PATCH',
      headers: getAuthHeaders(),
    ...
    if (!response.ok) {
      const errorText = await response.text()
      console.error('❌ Update file metadata error:', response.status, errorText)
  ...
  async updateFileSelection(fileId: string, selected: boolean): Promise<void> {
    console.log('☑️ API Request - Update File Selection:', { fileId, selected })

    const response = await fetch(`${API_BASE_URL}/files/${fileId}/selection?selected=${selected}`, {
      method: 'PATCH',
      headers: getAuthHeaders()
    })
    ...
    if (!response.ok) {
      const errorText = await response.text()
      console.error('❌ Update file selection error:', response.status, errorText)
  ...
  async getAvailableModels(): Promise<{models: string[]}> {
    ...
    if (!response.ok) {
      const errorText = await response.text()
      console.error('❌ Get available models error:', response.status, errorText)
  ...
  async getArtifacts(messageId: string): Promise<{message_id: string, artifacts: any[]}> {
    console.log('🎨 API Request - Get Artifacts:', { messageId })

    const response = await fetch(`${API_BASE_URL}/chat/${messageId}/artifacts`, {
      method: 'GET',
      headers: getAuthHeaders(),
    })
...
export const getLastUsedModel = (messages: MessageHistoryResponse[], availableModels: string[]): string => {
